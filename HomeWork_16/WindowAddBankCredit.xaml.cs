...
namespace HomeWork_16
{
    ...
    public partial class WindowAddBankCredit : Window
    {
        ...
        private void AddBankCredit(object sender, RoutedEventArgs e)
        {
            ...
            var bankCredit = new BankCredit(credit, Bank.Date, countYears, bankAccount, sum);
            try
            {
                client.AddBankCredit(bankCredit, sum);


                Bank.StartActionLogs(
                    $"Клиент {client.FullName} взял кредит №{bankCredit.Number} на " +
                    $"сумму {sum} от {bankCredit.DateOpen:dd.MM.yyyy}."
                );
            }
